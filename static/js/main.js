(function($){
  ...
  $('#submit-button').click(function(){
      ...
      $.ajax({
            ...
            success: function(result) {
              ...
              // $('#error_list').html(response['error_ass']);
              //$('#error_simulate').html(str(response['error_ass']);
              $('#stack').html("Stack");
              tabs = "";
              tabs += '<div class="row" style="padding-top:10%;"><div class="col s12" tab-indicator-black><ul class="tabs tabs-fixed-width ">';
              for(i=0;i<fileNames.length;i++){
                tabs += ' <li class=" col"><a class="btn black lighten-3 white-text text-darken-4" href="#filetab'+i+'">'+fileNames[i]+'</a></li>';
              }
              tabs += '</ul></div>';
              for(i=0;i<fileNames.length;i++){               
                tabs += '<div id="filetab'+i+'" class="col s12">';
                tabs+= '<div class="col s4 offset-s4 card-panel black lighten-4 hoverable white-text" >'+response['filedata'][fileNames[i]].replace(/\n/g,"<br>")+'</div>';
                tabs+='</div>';
              }
              tabs += '</div>';
              $('#file-data').html(tabs);
              ...
              for(i=0;i<fileNames.length;i++){
                tabs += ' <li class="tab col"><a class="grey lighten-3 grey-text text-darken-4" href="#pass1tab'+i+'">'+fileNames[i]+'</a></li>';
              }                    
              ...
              for(i=0;i<fileNames.length;i++){
                ...
                tabs+= '<div class="col s4 offset-s2 card-panel black lighten-4 hoverable white-text" >'+response['pass1'][fileNames[i].split('.')[0]].replace(/\n/g,"<br>")+'</div>';
                tabs+= '<div id="tables" class="row col s4" style="display:block">';
                tabs+='<div class="col s12 card-panel black lighten-4 hoverable white-text" > Symbols Table<br>'+printDic(response['symTable'][tempname])+'</div>';
                tabs+='<div class="col s12 card-panel black lighten-4 hoverable white-text" > Literals Table<br>'+printRealDic(response['litTable'][tempname])+'</div>';
                tabs+='<div class="col s12 card-panel black lighten-4 hoverable white-text" > Global Table<br>'+printDic(response['globTable'][tempname])+'</div>';
                tabs+= '</div>';
                tabs+='</div>';
              }
              tabs += '</div>';
              ...
              tabs += '<div class="row"><div class="col s12"><ul class="tabs tabs-fixed-width">';
              for(i=0;i<fileNames.length;i++){
                tabs += ' <li class="tab col"><a class="grey lighten-3 grey-text text-darken-4" href="#pass2tab'+i+'">'+fileNames[i]+'</a></li>';
              }                    
              ...
              for(i=0;i<fileNames.length;i++){
                ...
                tabs+= '<div class="col s4 offset-s2 card-panel black lighten-4 hoverable white-text " >'+response['pass2'][fileNames[i].split('.')[0]].replace(/\n/g,"<br>")+'</div>';
                tabs+= '<div id="tables" class="row col s4" style="display:block">';
                tabs+='<div class="col s12 card-panel  black lighten-4 hoverable white-text" > Symbols Table<br>'+printDic(response['symTable'][tempname])+'</div>';
                tabs+='<div class="col s12 card-panel  black lighten-4 hoverable white-text" > Literals Table<br>'+printRealDic(response['litTable'][tempname])+'</div>';
                tabs+='<div class="col s12 card-panel  black lighten-4 hoverable white-text" > Global Table<br>'+printDic(response['globTable'][tempname])+'</div>';
  ...
  // $('#error_details').html(str(response['error_ass'])
  // $('#error_details').click(function(){
  //   $.ajax({
  //       type : "POST",
  //       url : "/runSimulator",
  //       // data: JSON.stringify({file: fileNames[0].split('.')[0]}),
  ...
  //         // $('#currentInst').html('<b> CURRENT INSTRUCTION: </b>'+ response['memory'][response['reg']['PC']]);
  //         // $('#stack').html(stackString(response['stack']));
  //         $('#error_list').html(str(response['error_ass'])
...
// console.log("\n\n\n\ni was here     \n \n")
//           console.log(response['error_ass'])
//           $('#error_simulate').html('<b>ERROR</b><br>'+str(response['error_ass']));